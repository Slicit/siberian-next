...
- The build worker still sends its artifact through the Mobile service and Storage, which is the 67 MB the presigned upload path exists to save. Wiring it means Mobile minting an address, the builder writing to it, and Mobile confirming: a protocol change between the two, verifiable only by a real Android build, which is the most disk-hungry thing the box does. (trigger: out-of-scope, source: feat-presigned-uploads decision 2026-08-29, agent: claude-code)
