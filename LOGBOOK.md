...
- Garage for object storage, as the first backend behind an S3 compatible facade rather than as a fixed choice. It is the only container holding objects, and the only credential for it belongs to the Storage service. The Router joins that network too, to forward already signed requests at `s3.<domain>`: it holds no credential and can sign nothing, so it reaches the object store without being able to read it. A managed S3 replaces the container and the door, and nothing above the facade notices
