...
# domain. A module has no field in which to name another module's mail.
module ModuleAuthentication
  ...
  # Scoped at the source. Every query in this service starts here, so there is
  # no path by which one module reaches another module's mail.
  def scope
    current_module.messages.where(domain: current_domain)
  end
end
