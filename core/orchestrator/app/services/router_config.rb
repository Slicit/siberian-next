...
class RouterConfig
  ...
  MAP_FILENAME = "00-upstreams.conf"
  ...
  # One map for every installed module, its name to its upstream.
  #
  ...
  # answer it. A map is the nginx construct for exactly this. Rewritten whole
  # rather than appended to, so a removed module leaves nothing behind.
  def refresh_upstreams!(installed_modules)
    FileUtils.mkdir_p(@config_dir)

    lines = Array(installed_modules).filter_map do |installed|
    ...
    body = +"# Generated by the Orchestrator. An unknown module name maps to nothing,\n"
    body << "# and the Router answers 404 rather than proxying somewhere.\n"
    body << "map $module_name $module_upstream {\n"
    body << "  default \"\";\n"
    lines.each { |line| body << "#{line}\n" }
    body << "}\n"

    File.write(File.join(@config_dir, MAP_FILENAME), body)
  end
